...
import { mockPopulationZones, mockShelters, mockRoads, roadWaypoints, demoRoutes, DEMO_USER_LOCATION } from '@/lib/services/mockData';
...
// Shelter icon color based on VACANCY percentage (green=lots of space, red=full)
const createShelterIcon = (shelter: typeof mockShelters[0]) => {
  ...
  let color: string;
  if (vacancyPct <= 0) color = '#dc2626';       // red — FULL
  else if (vacancyPct <= 10) color = '#ef4444';  // red-ish — near capacity
  else if (vacancyPct <= 30) color = '#f97316';  // orange
  else if (vacancyPct <= 60) color = '#eab308';  // yellow
  else color = '#22c55e';                         // green — plenty of space
...
export default function MapContent() {
  ...
  const { assignments, projected_loads } = planState;
  ...
  return (
    <div className="h-full w-full">
      <MapContainer center={center} zoom={13} scrollWheelZoom={true} zoomControl={false} style={{ height: '100%', width: '100%', zIndex: 0 }}>
              ...
              {mapLayers.riskZones && (
                <Circle
                  ...
                  pathOptions={{
                    ...
                    fillOpacity: 0.10, weight: 1.5, opacity: 0.5
                  }}
                />
              )}
            </LayerGroup>
              ...
              <Popup maxWidth={300}>
                <div style={{ fontSize: '11px', lineHeight: '1.4', minWidth: '240px' }}>
                  ...
                  <table style={{ width: '100%', marginBottom: '8px' }}>
                    <tbody>
                      <tr><td style={{ color: '#64748b', paddingRight: '12px' }}>Total Beds</td><td style={{ fontWeight: 700 }}>{shelter.total_capacity.toLocaleString()}</td></tr>
        ...
        {/* === EVACUATION ROUTES (using realistic road waypoints) === */}
        {mapLayers.routes && assignments.map((assignment, idx) => {
          ...
          // Find matching road for waypoints
          const road = mockRoads.find(r => r.source_zone_id === assignment.zone_id && r.target_shelter_id === assignment.shelter_id);
          const waypoints = road && roadWaypoints[road.id]
            ? roadWaypoints[road.id]
            : [[zone.location.lat, zone.location.lng], [shelter.location.lat, shelter.location.lng]] as [number, number][];
              ...
              >
                <Popup>
                  <div style={{ fontSize: '11px' }}>
                    ...
                    <div style={{ color: '#64748b', marginTop: '4px' }}>This road is currently impassable due to flooding.</div>
                  </div>
                </Popup>
              </Polyline>
            );
        ...
        {/* === DEMO: Unsafe (Red) vs Safe (Green) Route === */}
        <Polyline
          positions={demoRoutes.unsafe.waypoints}
          pathOptions={{ color: '#ef4444', weight: 4, dashArray: '10,6', opacity: 0.85 }}
        >
          <Popup maxWidth={260}>
            <div style={{ fontSize: '11px' }}>
              <div style={{ fontWeight: 700, color: '#ef4444', marginBottom: '4px' }}>⚠️ {demoRoutes.unsafe.label}</div>
              <div style={{ color: '#64748b' }}>{demoRoutes.unsafe.reason}</div>
        ...
        <Polyline
          positions={demoRoutes.safe.waypoints}
          pathOptions={{ color: '#22c55e', weight: 5, opacity: 0.9 }}
        >
          <Popup maxWidth={260}>
            <div style={{ fontSize: '11px' }}>
              <div style={{ fontWeight: 700, color: '#16a34a', marginBottom: '4px' }}>✅ {demoRoutes.safe.label}</div>
              <div style={{ color: '#64748b' }}>{demoRoutes.safe.reason}</div>
              <div style={{ marginTop: '6px', fontWeight: 700, color: '#16a34a', fontSize: '10px' }}>RECOMMENDED SAFE ROUTE</div>
            </div>
          </Popup>
        </Polyline>

        {/* === IMPACT ZONES (during simulation preview) === */}
